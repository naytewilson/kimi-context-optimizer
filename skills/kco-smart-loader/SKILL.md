---
name: kco-smart-loader
description: Automatically suggests optimal files to preload based on the user's task description and historical context patterns
whenToUse: When the user starts a new task, mentions reading files, or when session context is being set up
...
```bash
ls ~/.kimi-context-optimizer/templates/ 2>/dev/null
```
